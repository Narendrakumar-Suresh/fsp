...
from data_gen import DRUGS, generate_all, extract_features, HIGH_RISK_TRIPLES
from causal import run_causal_pipeline
from model import ADRNet, train_local, mc_predict, fit_temperature
from attribution import PatientRecord, leave_one_out, detect_synergies
...
# ── Train model ────────────────────────────────────────────────────────────
console.print("\n[bold cyan]Training model on synthetic hospital data...[/bold cyan] ", end="")
df_all = generate_all(n_per_hospital=800)
...
X_tr, X_te, y_tr, y_te = train_test_split(X_aug, y_all, test_size=0.2, random_state=42, stratify=y_all)
model = ADRNet(input_dim=X_aug.shape[1])
train_local(model, X_tr, y_tr, epochs=80)
...
while True:
    ...
    unknown    = [d.strip() for d in raw.split(",") if d.strip() not in DRUGS and d.strip()]
    ...
    x_basic  = np.array([age, n_conds, len(drug_names), creatinine, 6.8, 4.3], dtype=np.float32)
    ...
    alert = "CRITICAL" if risk >= 0.8 else "HIGH" if risk >= 0.6 else "MODERATE" if risk >= 0.4 else "LOW"
    color = {"CRITICAL":"bold red","HIGH":"red","MODERATE":"yellow","LOW":"green"}[alert]

    # Detected known triples
    drug_set = set(drug_names)
    active_triples = [f"{a}+{b}+{c}" for a,b,c in HIGH_RISK_TRIPLES if {a,b,c}.issubset(drug_set)]
    triple_line = f"\n[bold red]⚠ HIGH-RISK TRIPLE: {', '.join(active_triples)}[/bold red]" if active_triples else ""

    console.print(Panel(
        f"[bold]Regimen:[/bold] {', '.join(drug_names)}{triple_line}\n\n"
        f"[bold]ADR Risk Score:[/bold]  [{color}]{risk:.1%}[/{color}]  [{color}]{alert}[/{color}]\n"
        f"[bold]95% CI:[/bold]          [{ci_lo:.1%} – {ci_hi:.1%}]\n\n"
    ...
    for drug in sorted(loo, key=lambda d: -loo[d]):
        delta = loo[drug]
        role  = "⚠ PRIMARY CULPRIT" if delta > 0.10 else "↑ Adds Risk" if delta > 0.02 else "↓ Protective" if delta < -0.02 else "Neutral"
        c     = "bold red" if delta > 0.10 else "red" if delta > 0.02 else "green" if delta < -0.02 else "white"
        t.add_row(drug, f"[{c}]{delta:+.1%}[/{c}]", role)
    console.print(t)

    # Synergy
    console.print("[dim]Detecting synergies...[/dim]")
    synergies = detect_synergies(model, patient, n_passes=10, top_k=3, temperature=T_star)
    if synergies:
        ...
        for syn in synergies:
            c = "bold red" if syn["synergy_score"] > 0.1 else "red" if syn["synergy_score"] > 0.05 else "yellow"
            s.add_row(" + ".join(syn["drugs"]), str(syn["order"]),
                      f"{syn['joint_risk']:.1%}", f"[{c}]{syn['synergy_score']:+.4f}[/{c}]")
        console.print(s)
    ...
    console.print(Panel(
        ...
        title="[bold]Clinical Summary[/bold]", border_style="cyan"))
